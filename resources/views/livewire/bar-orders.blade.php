<div wire:poll.10000ms>
    ...
    <div class="grid grid-cols-3 gap-4 mt-2 text-white">
        @foreach ($orderItems as $orderItem)
            <div tabindex="0" wire:keydown.enter="confirmingMark({{ $orderItem }})" class="focus:ring-4 ring-offset-2 ring-offset-slate-50 flex flex-col relative focus-within:shadow-lg justify-between p-4 rounded-lg {{ OrderItemsStatus::from($orderItem->status)->color() }} shadow-lg w-full cursor-pointer" wire:click="confirmingMark({{ $orderItem }})">
                ...
                <hr class="my-2">
                <div class="text-center flex-none">@hour($orderItem->created_at)</div>
    ...
    <x-dialog-modal wire:model.live="confirmMark">
        ...
        <x-slot name="content">
            ...
            <div class="text-right">
                @if ($confirmItem->status == OrderItemsStatus::Assessing->value || $confirmItem->status == OrderItemsStatus::Preparing->value)
                    @if ($confirmItem->status == OrderItemsStatus::Assessing->value)
                        <x-button wire:click="setRejected" class="bg-red-500 text-black">Rejeitar Pedido</x-button>
                    @else 
            ...
            <div class="flex flex-col relative justify-between p-4 rounded-lg {{ OrderItemsStatus::from($confirmItem->status)->color() }} shadow-lg w-full">
                ...
                <hr class="my-2">
                <div class="text-center flex-none">@hour($confirmItem->created_at)</div>
            ...
            <div class="text-center">
                @if ($confirmItem->status == OrderItemsStatus::Assessing->value)
                    <x-button wire:click="setPreparing" class="{{ OrderItemsStatus::Preparing->color() }}" autofocus>Aceitar Pedido</x-button>
                @elseif ($confirmItem->status == OrderItemsStatus::Preparing->value)
                    <x-button wire:click="setConcluded" class="bg-green-500" autofocus>Marcar como Concluído</x-button>
                @endif
            </div>
            @endif
        </x-slot>
